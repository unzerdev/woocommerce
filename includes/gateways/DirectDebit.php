...
In case of refusal or rejection of direct debit payment I instruct my bank irrevocably to inform %merchant% or any third party upon request about my name, address and date of birth.',
			'unzer-payments'
		);
		parent::__construct();
	...
	public function process_payment( $order_id ) {
		...
		$directDebitId = sanitize_text_field( wp_unslash( ! empty( $selectedSavedPaymentInstrument ) ? $selectedSavedPaymentInstrument : Util::getNonceCheckedPostValue( 'unzer-direct-debit-id' ) ) );
		...
		if ( ! ( $charge->isPending() || $charge->isSuccess() ) ) {
			throw new Exception( esc_html( $charge->getMessage()->getCustomer() ) );
		}
		if ( $charge->isSuccess() ) {
